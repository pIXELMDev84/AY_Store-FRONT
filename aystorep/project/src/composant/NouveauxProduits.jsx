...
function NouveauxProduits() {
  ...
  const filteredProduits = nouveauxProduits.filter(produit =>
  ...
  useEffect(() => {
    const fetchNouveauxProduits = async () => {
      setIsLoading(true);
      try {
        const response = await fetch('http://127.0.0.1:8000/api/produits'); // Change the endpoint as needed
        const data = await response.json();
        ...
        const nouveauxProduits = data.filter(produit => {
          const productDate = new Date(produit.created_at);
          return productDate >= oneDayAgo;
        });

        setNouveauxProduits(nouveauxProduits);
      } catch (error) {
